        ...
        <form method="POST" action="{{ route('manager.positions.store') }}">
            ...
            <div style="display: flex; gap: 20px;">
                <div class="form-group" style="flex: 1;">
                    ...
                    <input type="text" name="luong_co_ban" class="form-control format-money" value="{{ old('luong_co_ban') }}">
                    @error('luong_co_ban')
                        <div class="form-error">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group" style="flex: 1;">
                    ...
                    <input type="text" name="luong_theo_gio" class="form-control format-money" value="{{ old('luong_theo_gio') }}">
...
document.addEventListener('DOMContentLoaded', function() {
    const formatMoney = (val) => {
        ...
        if (!number) return '';
        // Định dạng có dấu phẩy cho 3 chữ số
