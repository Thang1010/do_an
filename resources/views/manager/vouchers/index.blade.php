...
@section('breadcrumb', 'Kinh doanh / <strong>Voucher & Khuyến mãi</strong>')

@section('content')

<div class="page-header">
    <div>
        <h1 class="page-title">Voucher & Khuyến mãi</h1>
        <p class="page-subtitle">Quản lý mã giảm giá và chương trình ưu đãi</p>
    </div>
    <div class="page-actions">
        <a href="{{ route('manager.vouchers.create') }}" class="btn btn-primary">Tạo voucher mới</a>
    </div>
</div>
...
<div class="grid-3 mb-20">
    <div class="stat-card">
        <div class="stat-label">Voucher đang hoạt động</div>
    ...
    <div class="stat-card">
        <div class="stat-label">Voucher sắp hết hạn</div>
...
<div class="filter-bar">
    <form method="GET" action="{{ route('manager.vouchers.index') }}" class="flex-gap-10">
        <input type="text" name="search" class="form-control filter-search"
               placeholder="Tìm mã hoặc tên voucher..." value="{{ request('search') }}">
        <select name="trang_thai" class="form-control w-200">
            <option value="">Tất cả trạng thái</option>
            <option value="đang hoạt động" {{ request('trang_thai') === 'đang hoạt động' ? 'selected' : '' }}>Đang hoạt động</option>
            <option value="ngưng hoạt động" {{ request('trang_thai') === 'ngưng hoạt động' ? 'selected' : '' }}>Ngưng hoạt động</option>
            <option value="chưa phát hành" {{ request('trang_thai') === 'chưa phát hành' ? 'selected' : '' }}>Chưa phát hành</option>
...
<div class="card">
    <div class="card-header">
        <span class="card-title">Danh sách Voucher</span>
    </div>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $voucher->don_toi_thieu > 0 ? number_format($voucher->don_toi_thieu, 0, ',', '.') . 'đ' : '—' }}</td>
                    ...
                    <td>
                        <div class="action-row">
                                                        <a href="{{ route('manager.vouchers.show', $voucher->id) }}" class="btn btn-primary btn-sm">Chi tiết</a>
                                                        <a href="{{ route('manager.vouchers.edit', $voucher->id) }}" class="btn btn-secondary btn-sm">Sửa</a>
                            <form method="POST" action="{{ route('manager.vouchers.destroy', $voucher->id) }}"
                                  onsubmit="return confirmDelete(this, 'Xóa voucher {{ $voucher->ma_voucher }}?')">
                                @csrf
                                @method('DELETE')
...
<div class="card-footer" style="border-top: 0;">
    <div class="pagination-footer">
        <span class="pagination-info">
            Hiển thị {{ $vouchers->firstItem() }}-{{ $vouchers->lastItem() }} / {{ $vouchers->total() }} voucher
        </span>
        {{ $vouchers->links() }}
    </div>
</div>
